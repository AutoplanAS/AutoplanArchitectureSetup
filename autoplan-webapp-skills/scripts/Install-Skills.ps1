<#
.SYNOPSIS
    Installs the Autoplan webapp skills into one or more AI agents' skills directories.

.DESCRIPTION
    Links every folder under skills/ matching autoplan-webapp-* into each selected agent's
...
if (-not (Test-Path $sourceRoot)) {
    throw "No skills directory found at '$sourceRoot'. Run this script from a clone containing autoplan-webapp-skills."
}

$skills = Get-ChildItem -Path $sourceRoot -Directory |
    Where-Object { $_.Name -like 'autoplan-webapp-*' -and (Test-Path (Join-Path $_.FullName 'SKILL.md')) }

if (-not $skills) {
    throw "No autoplan-webapp-* skills found under '$sourceRoot' (a skill folder must contain SKILL.md)."
}
